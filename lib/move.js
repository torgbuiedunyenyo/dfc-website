...
// Human title for the Future content — first heading, else first paragraph
// with text. Used for admin button labels and default project titles.
function futureTitle(futureHtml) {
  const $ = cheerio.load(`<div id="src">${futureHtml || ''}</div>`);
  const nodes = $('#src').find('h1,h2,h3,p').toArray();
...
module.exports = { venueHeader, clearedColumn, futureToColumn, futureTitle, CLEARED_FUTURE };
