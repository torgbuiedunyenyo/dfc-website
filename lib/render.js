...
function removeEmptyCurrentSections($, contentMap) {
  $('.current-exhibition[data-cms]').each((_, el) => {
    ...
    if (id) $(`.current-section-links a[href="#${id}"]`).remove();
    section.remove();
  });
  if (!$('.current-section-links a').length) $('.current-section-links').remove();
}
...
async function renderPage(client, name, { editMode = false } = {}) {
  ...
  ensureFooterAddress($);
  if (name === 'Current' && !editMode) removeEmptyCurrentSections($, contentMap);
