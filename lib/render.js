...
function pastGridHtml(projects) {
  const preferredOrder = ['Exhibitions + Residencies', 'Selected Talks + Workshops', 'Other Events', 'More from the archive'];
  const groups = new Map();
  for (const project of projects) {
    const category = project.category || (project.status === 'archive' ? 'More from the archive' : 'Past Projects');
...
async function renderPage(client, name, { editMode = false } = {}) {
  ...
  if (name === 'Past') {
    const projects = await client`
      ...
      WHERE status IN ('past', 'archive') AND length(trim(body_html)) > 0
      ORDER BY CASE WHEN status = 'past' THEN 0 ELSE 1 END, sort_order ASC, id ASC`;
