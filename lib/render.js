...
const ABOUT_ORIGIN = 'Founded by Ann Schnake, Robert Gomez Hernandez, and Stacey Goodman in 2018,';
const VISIT_ELEVATOR_NOTE = 'Although an elevator serves the basement restrooms, there is one step up to the elevator.';
...
function applyPagePresentation($, name) {
  if (name === 'About') {
    const $intro = $('[data-cms="about.intro"]');
    const html = $intro.html() || '';
    if (!html.includes(ABOUT_ORIGIN)) {
      $intro.html(html.replace('Originally founded in 2018,', ABOUT_ORIGIN));
    }
  }

  if (name === 'Current') {
    $('[data-cms="current.column-1"]').find('h1, h2, h3').first().text('IN MAIN GALLERY');
  }

  if (name === 'Visit') {
    const $body = $('[data-cms="visit.body"]');
    if (!$body.text().includes(VISIT_ELEVATOR_NOTE)) {
      const $note = $(`<p class="visit-access-note">${VISIT_ELEVATOR_NOTE}</p>`);
      const $image = $body.find('img').first();
      if ($image.length) $note.insertBefore($image);
