...
function loadUses() {
  ...
  for (const entry of manifest.pages) {
    ...
    for (const media of page.media || []) {
      ...
      if (!uses.has(sourceUrl)) uses.set(sourceUrl, []);
      uses.get(sourceUrl).push({
...
async function main() {
  ...
  async function worker() {
    while (cursor < urls.length) {
      ...
      try {
        const record = await download(sourceUrl);
        entries[sourceUrl] = record;
        process.stdout.write(`downloaded ${index + 1}/${urls.length} ${record.filename}\n`);
      } catch (error) {
        missing.push({ source_url: sourceUrl, error: error.message, uses: uses.get(sourceUrl) });
  ...
  const mediaMap = {
    ...
    downloaded_count: Object.keys(entries).length,
    unavailable_count: missing.length,
    total_bytes: Object.values(entries).reduce((sum, item) => sum + item.bytes, 0),
    entries,
  };
